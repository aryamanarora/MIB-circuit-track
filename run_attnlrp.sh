...
for cell in "${CELLS[@]}"; do
  ...
  name="v-attnlrp-${task}-${model}"
  cmd="$pp; \
$PY run_attribution.py --models $model --tasks $task --method AttnLRP --level node --ablation patching --split train --batch-size $abatch $nf --circuit-dir results/attnlrp && \
$PY run_evaluation.py --models $model --tasks $task --method AttnLRP --level node --ablation patching --split validation --batch-size $abatch $hf --circuit-dir results/attnlrp --output-dir results/attnlrp_eval"
