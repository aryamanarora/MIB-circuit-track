...
for cell in "${CELLS[@]}"; do
  ...
  name="v-relp-${task}-${model}"
  cmd="$pp; \
$PY run_attribution.py --models $model --tasks $task --method RelP --level node --ablation patching --split train --batch-size $abatch $nf --circuit-dir results/relp && \
$PY run_evaluation.py --models $model --tasks $task --method RelP --level node --ablation patching --split validation --batch-size $abatch $hf --circuit-dir results/relp --output-dir results/relp_eval"
